...
static void tprintnodes_level(tree* t, slist* q) {
    tree_printme(t, slist_front(q));
    if (slist_front(q)->left != NULL) slist_push(q, slist_front(q)->left);
    if (slist_front(q)->right != NULL) slist_push(q, slist_front(q)->right);
    slist_pop(q);
    if (!slist_empty(q)) tprintnodes_level(t, q);
}

void tprint_levelorder(tree* t) {
    if (t->root == NULL) return;
    slist* list = slist_create();
    slist_push(list, t->root);
    tprintnodes_level(t, list);
    printf("\n");
}
void tree_test() {
    tree* t = tree_create();
